...
def _exporting_impl(
    ...
    file_type = onnx_path.split('.')[-1]
    assert file_type in ['onnx', 'ts']
    assert precision in [torch.float, torch.half]

    save_dir = os.path.dirname(onnx_path)
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)

    x = torch.randn((1, 3, net.input_size, net.input_size), dtype=precision)
    ...
    if file_type == 'ts':
        ...
        inputs = [
            torch_tensorrt.Input(
                min_shape=[1, 3, net.input_size, net.input_size],
                opt_shape=[1, 3, net.input_size, net.input_size],
                max_shape=[1, 3, net.input_size, net.input_size],
                dtype=precision,
            )
        ]

        trt_ts_module = torch_tensorrt.compile(
            net, inputs=inputs, enabled_precisions={precision},
            require_full_compilation=True
        )

        result = trt_ts_module(x.cuda())
        torch.jit.save(trt_ts_module, onnx_path)
