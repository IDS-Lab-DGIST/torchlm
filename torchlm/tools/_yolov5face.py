...
from ..tools.yolov5face.common import Conv, Bottleneck, SPP, DWConv, Focus, BottleneckCSP, C3, ShuffleV2Block, Concat, NMS, autoShape, StemBlock, BlazeBlock, DoubleBlazeBlock
from ..tools.yolov5face.experimental import MixConv2d, CrossConv, attempt_load
from ..tools.yolov5face.autoanchor import check_anchor_order
from ..tools.yolov5face.general import make_divisible, check_file, set_logging, check_img_size, non_max_suppression_face
from ..tools.yolov5face.torch_utils import time_synchronized, fuse_conv_and_bn, model_info, scale_img, initialize_weights, \
    select_device, copy_attr
from ..tools.yolov5face.datasets import letterbox
...
import sys
sys.path.insert(0, './torchlm/tools/yolov5face')


class YOLOv5Face(FaceDetBase):
    def __init__(self):
        super().__init__()
        cfg='./torchlm/tools/yolov5face/configs/yolov5s.yaml'
        pt = './torchlm/tools/yolov5face/models/yolo/yolov5s.pt'
        self.modules = Model(cfg=cfg)
        # self.modules = attempt_load(pt, map_location=torch.device('cpu'))
        # sd = torch.load(pt)['models'].float().state_dict()  # to FP32
        # self.modules.load_state_dict(torch.load(pt, map_location=torch.device('cpu')).float())
    ...
    def apply_detecting(
        ...
        orgimg = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
        img0 = deepcopy(orgimg)
        ...
        imgsz = check_img_size(img_size, s=self.modules.stride.max())  # check img_size
...
class Model(nn.Module):
    def __init__(self, cfg='./torchlm/tools/yolov5face/configs/yolov5s.yaml', ch=3, nc=None):  # model, input channels, number of classes
